...
for i = 1:length(2)
    ...
    start_time = combineBytesToDecimal(element,3,4,5,14)*tick;
    ...
    diff(mac_id,recPAckets(mac_id)) = time;
    pre_time = time;
        if(mac_id==1)
...
time_1=TimeArrayGenerator(transTime_1,count(1));
time_2=TimeArrayGenerator(transTime_2,count(2));
time_3=TimeArrayGenerator(transTime_3,count(3));
time_4=TimeArrayGenerator(transTime_4,count(4));
time_5=TimeArrayGenerator(transTime_5,count(5));
time_6=TimeArrayGenerator(transTime_6,count(6));
time_7=TimeArrayGenerator(transTime_7,count(7));
time_8=TimeArrayGenerator(transTime_8,count(8));
...
pirA= transpose(tempPirA(1:(count(1)-1)*32));
pirB= transpose(tempPirB(1:(count(2)-1)*32));
pirC= transpose(tempPirC(1:(count(3)-1)*32));
pirD= transpose(tempPirD(1:(count(4)-1)*32));
pirE= transpose(tempPirE(1:(count(5)-1)*32));
pirF= transpose(tempPirF(1:(count(6)-1)*32));
pirG= transpose(tempPirG(1:(count(7)-1)*32));
pirH= transpose(tempPirH(1:(count(8)-1)*32));
...
for i = 1:length(2)
    ...
    diff(mac_id,recPAckets(mac_id)) = time-pre_time(mac_id);
